...
import { resolveCatalogPlan } from "@/lib/services/catalog-service";
...
export async function getFinanceSummary() {
  const [payments, pending, completed, products] = await Promise.all([
    prisma.payment.findMany({ orderBy: { createdAt: "desc" }, take: 50, include: { client: true, license: true } }),
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (payment?.licenseId) {
      await prisma.licenseRecord.update({
        ...
        data: { paymentStatus: STATUS_ACTIVE },
      });
    }
  }
}
